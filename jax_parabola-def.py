...
def MAP(
        ...
        params=jnp.stack((start['p_a'],start['p_b'],start['p_c'])).T
        ...
        def update(params, opt_state):
            ...
            splt_params = np.reshape(splt_params,(dev_cnt,n_samples//dev_cnt,3))
            grads = g2(splt_params)
            grads1=np.reshape(grads,(dev_cnt*(n_samples//dev_cnt),3))
...
def f(x):
  return TRUE_A * x ** 2 + TRUE_B * x + TRUE_C
...
def logprior(theta):
    ...
    p_b=tfd.Normal(5,3)
    p_c=tfd.Normal(4,3)
    return p_a.log_prob(theta[0])+p_b.log_prob(theta[1])+p_c.log_prob(theta[2])

def loglikelihood(theta):
    predicted_y=theta[0] * x ** 2 + theta[1] * x + theta[2]
...
plt.plot(x, theta_map[0] * x ** 2 + theta_map[1] * x + theta_map[2] , label="MAP Predictions")
plt.legend()
plt.show()
...
plt.figure()
plt.plot(losses)
plt.savefig('/global/homes/d/davalv03/parabola_plots/losses.png')
...
plt.plot(x, y, '.', label="Data")
plt.plot(x, f(x), label="Ground truth")
plt.plot(x, theta_svi[0] * x ** 2 + theta_svi[1] * x + theta_svi[2] , label="SVI Predictions")
plt.legend()
plt.show()
plt.savefig('/global/homes/d/davalv03/parabola_plots/plot_SVI.png')
...
figure=corner(s_R2, show_titles=True,truths=TRUE_VALUES,labels=['A', 'B', 'C'])
plt.show()
plt.savefig('/global/homes/d/davalv03/parabola_plots/corner_plot.png')
plt.figure()
...
for i in range(4):
 ...
 plt.title('Quadratic coefficient trace')
...
plt.tight_layout()
plt.show()
plt.savefig('/global/homes/d/davalv03/parabola_plots/trace_quadratic.png')
plt.figure()
...
for i in range(4):
 ...
 plt.title('Linear coefficient trace')
...
plt.tight_layout()
plt.show()
plt.savefig('/global/homes/d/davalv03/parabola_plots/trace_linear.png')
plt.figure()
...
for i in range(4):
 ax1.plot(samples_res[:,i,2], colors[i], label = f'{i}', alpha = 0.6)
 plt.title('Independent coefficient trace')
...
ax2 = plt.subplot(gs[0, 1], sharey=ax1)
for i in range(4):
  prob, bins = np.histogram(samples_res[:,i,2], density = True, bins = 40)
  (mu, sigma) = norm.fit(samples_res[:,i,2])
...
plt.tight_layout()
plt.show()
plt.savefig('/global/homes/d/davalv03/parabola_plots/trace_independent.png')
plt.figure()